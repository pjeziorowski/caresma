...
└─────────┼─────────────────┼────────────────┼───────────────────────────────┘
          ...
          ▼                 ▼                │                 │
┌─────────────────────────────────────────────────────────────────────────────┐
│  HONO API (SvelteKit route) OR separate NestJS app — see §2.1 below       │
│                                                                             │
...
**Backend architecture:** The entire backend is a single Hono application structured as a **modular monolith following Domain-Driven Design (DDD)** principles — each bounded context (auth, conversation, assessment, reports, scheduling, notifications) is organized as a separate Hono route module with its own domain logic, entities, value objects, and repository interfaces. This keeps business rules encapsulated and testable independent of infrastructure. The application is mounted as a SvelteKit API route (`/api/[...path]`) for zero-friction deployment as part of the web app. Because Hono is runtime-agnostic, this same app can be extracted into a standalone Cloudflare Worker, Bun server, or Node service with no code changes when scaling requires it. The system could equally be built with a more mature stack (e.g. NestJS + React); for MVPs and small teams, the preference here is SvelteKit/Hono for its pragmatic, fast path from idea to production.

### 2.1 API implementation: Hono vs. separate NestJS app

The diagram and description above assume a **Hono** API mounted as a SvelteKit API route. Alternatively, the API may be implemented as a **separate NestJS application**. The same logical architecture (middleware, STT → LLM → TTS, assessment engine, report generator, scheduler, notifications) applies in both cases; only the framework and deployment topology differ.

- **Hono + SvelteKit:** Single repo, single deploy, minimal ops. Ideal for small teams and MVP. DDD structure via Hono route modules.
- **Separate NestJS app:** Dedicated backend service, potentially different team or scaling path. NestJS offers built-in DI, modules, and a mature ecosystem; deployment would be independent (e.g. separate Cloudflare Worker, Node server, or container). More details on when and how to choose the NestJS option will be added later in this document.
...
| **Frontend**        | SvelteKit + Tailwind CSS                       | I prefer **SvelteKit** for small teams and MVPs — simpler mental model, less boilerplate, fast SSR, accessibility-first (ideal for elderly users on older devices). A more mature ecosystem like **React/Next.js** is also considerable depending on team size, hiring, or integration needs. |
...
| **Backend**         | Hono on SvelteKit server routes (DDD)          | Type-safe API layer with Zod validation, organized using **Domain-Driven Design (DDD)** — bounded contexts (conversation, assessment, reports, scheduling, notifications) map cleanly to Hono route modules, each owning its own domain logic, entities, and value objects. Initially deployed as SvelteKit server routes for simplicity — one repo, one deploy. Hono is framework-agnostic, so the API can be extracted into a standalone service (Cloudflare Worker, Bun, Node) with zero rewrite when scaling demands it. Built-in OpenAPI schema generation via `@hono/zod-openapi` makes it trivial to auto-generate typed HTTP clients for future mobile apps or IoT devices. Edge-deployed globally for <50ms TTFB. I prefer **SvelteKit/Hono** for small teams and MVPs; a more mature ecosystem like **NestJS** for the backend is also considerable depending on project plans. |
...
| **Cache**           | Cloudflare KV (or cache)                        | Serverless key-value store for conversation state, session context between turns, and rate limiting. Fits naturally if the app is deployed on Cloudflare Workers/Pages; no separate Redis vendor. For stricter consistency or Redis-specific features, Upstash Redis remains an option.                                                                                                                                                                                                                                                                                                                                   |
